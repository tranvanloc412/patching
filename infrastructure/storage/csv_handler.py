...
class CSVHandler:
        ...
        try:
            ...
            if fieldnames is None:
                if file_exists:
                    ...
                    existing_data = self.read_csv(file_path, delimiter=delimiter, quotechar=quotechar, encoding=encoding)
                    if existing_data:
            ...
            if file_exists:
                ...
                existing_content = self.file_storage.read_file(file_path, encoding=encoding)
                ...
                self.write_csv(file_path, data, fieldnames, delimiter, quotechar, encoding)
    ...
    def filter_csv(
        ...
        try:
            # Read input CSV
            data = self.read_csv(input_path, delimiter=delimiter, quotechar=quotechar, encoding=encoding)
            ...
            if filtered_data:
                fieldnames = list(data[0].keys())
                self.write_csv(output_path, filtered_data, fieldnames, delimiter, quotechar, encoding)
            else:
                # Create empty file with headers
                fieldnames = list(data[0].keys())
                self.write_csv(output_path, [], fieldnames, delimiter, quotechar, encoding)
            
            self.logger.info(f"CSV filtered: {input_path} -> {output_path} ({len(filtered_data)}/{len(data)} rows)")
    ...
    def merge_csv_files(
        ...
        try:
            ...
            self.write_csv(output_path, all_data, fieldnames, delimiter, quotechar, encoding)
            
            self.logger.info(f"CSV files merged: {len(input_paths)} files -> {output_path} ({len(all_data)} rows)")
            return len(all_data)
    ...
    def sort_csv(
        ...
        try:
            ...
            # Write sorted data
            fieldnames = list(data[0].keys())
            self.write_csv(output_path, sorted_data, fieldnames, delimiter, quotechar, encoding)
            
            self.logger.info(f"CSV sorted: {input_path} -> {output_path} ({len(sorted_data)} rows)")
    ...
    def validate_csv(
        ...
        try:
            ...
            try:
                sample_data = self.read_csv(file_path, delimiter=delimiter, quotechar=quotechar, encoding=encoding)
                if sample_data:
                    # Check for inconsistent row lengths
                    expected_columns = len(csv_info['column_names'])
                    for i, row in enumerate(sample_data[:100]):  # Check first 100 rows
                        if len(row) != expected_columns:
                            validation_result['warnings'].append(f"Row {i+2} has {len(row)} columns, expected {expected_columns}")
            except Exception as e:
                validation_result['valid'] = False
                validation_result['errors'].append(f"Failed to parse CSV data: {str(e)}")
