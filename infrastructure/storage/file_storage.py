...
class FileStorage:
    ...
    def delete_directory(self, directory_path: str, recursive: bool = False) -> bool:
        ...
        try:
            ...
            if path.exists():
                ...
                self.logger.warning(
                    f"Directory not found for deletion: {directory_path}"
                )
                return False
    ...
    def copy_file(self, source_path: str, destination_path: str) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to copy file {source_path} to {destination_path}: {str(e)}"
            )
            raise
    ...
    def list_directories(
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(directories)} directories in {directory_path}"
            )
            return sorted(directories)

        except Exception as e:
            self.logger.error(
                f"Failed to list directories in {directory_path}: {str(e)}"
            )
            raise
    ...
    def cleanup_old_files(
        ...
        try:
            self.logger.info(
                f"Cleaning up files older than {max_age_days} days in {directory_path}"
            )
    ...
    def create_backup(
        self, source_path: str, backup_directory: str, timestamp_suffix: bool = True
    ) -> str:
    ...
    def get_directory_size(self, directory_path: str) -> int:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get directory size for {directory_path}: {str(e)}"
            )
            raise

    def compress_directory(
        self, directory_path: str, output_path: str, format: str = "zip"
    ) -> str:
        ...
        try:
            ...
            if format.lower() == "zip":
                archive_path = shutil.make_archive(
                    str(output.with_suffix("")), "zip", str(src.parent), str(src.name)
                )
            ...
            self.logger.info(
                f"Directory compressed: {directory_path} -> {archive_path}"
            )
            return archive_path

        except Exception as e:
            self.logger.error(
                f"Failed to compress directory {directory_path}: {str(e)}"
            )
            raise
